...
const CATEGORIES = {
  dos_donts:  { label: "Do's & Don'ts", color: 'bg-info-50 text-info-700 ring-info-200', dot: '#3b82f6' },
  motivation: { label: 'Motivasi',      color: 'bg-purple-50 text-purple-700 ring-purple-200', dot: '#9333ea' },
  nutrition:  { label: 'Nutrisi',       color: 'bg-emerald-50 text-emerald-700 ring-emerald-200', dot: '#059669' },
  activity:   { label: 'Aktivitas',     color: 'bg-warning-50 text-warning-700 ring-warning-200', dot: '#d97706' },
}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-white rounded-xl border border-surface-200 h-72 animate-pulse" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filtered.map(m => {
            const cat = CATEGORIES[m.category] || { label: m.category, color: 'bg-surface-100 text-surface-600 ring-surface-200', dot: '#a3a39a' }
              ...
              <Card key={m.id} padding="none" hover className={`overflow-hidden flex flex-col ${!m.is_active ? 'opacity-60' : ''}`}>
                ...
                <div className="p-5 flex flex-col flex-1">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ring-1 ${cat.color}`}>
                  ...
                  <h3 className="font-semibold text-surface-800 mb-1 leading-snug">{m.title}</h3>
                  <p className="text-sm text-surface-500 leading-relaxed line-clamp-3 flex-1">{m.content}</p>

                  {/* Broadcast result feedback */}
                  {bResult && (
                    <div className={`mt-3 px-3 py-2 rounded-lg text-xs ${bResult.ok ? 'bg-primary-50 text-primary-700 border border-primary-200' : 'bg-danger-50 text-danger-700 border border-danger-200'}`}>
                      {bResult.ok
                  ...
                  <div className="flex items-center gap-2 mt-4 pt-3 border-t border-surface-100">
                    <button
                      ...
                      className="flex-1 py-2 text-xs font-medium text-white bg-primary-600 hover:bg-primary-700 disabled:opacity-50 rounded-lg transition-colors flex items-center justify-center gap-1.5"
                    >
                      {broadcasting === m.id ? (
                    ...
                    <button onClick={() => handleDelete(m.id)}
                      className="px-3 py-2 text-xs text-danger-400 hover:text-danger-600 hover:bg-danger-50 rounded-lg transition-colors">
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-surface-600 mb-1">Konten / Pesan *</label>
